...
### Deployment on Homelab server

Deployed on self-hosted Proxmox infrastructure with full Docker containerization. Cloudflare Tunnel provides secure external access without port forwarding, offering DDoS protection and automatic SSL/TLS encryption. The production environment uses Docker Compose to orchestrate the Go application, PostgreSQL, Redis, and Caddy reverse proxy with isolated networking and persistent volume storage.
...
Planned enhancements include developing an advanced AI system using Minimax with alpha-beta pruning. This will replace the current heuristic-based approach with game-tree search, enabling the bot to evaluate future game states and make optimal strategic decisions.
