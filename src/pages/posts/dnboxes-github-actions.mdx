...
I've been building a real-time multiplayer Dots and Boxes game in my spare time, and a few weeks ago I finally got it running in production on hardware sitting in my apartment. Every time I push to `main`, a GitHub Actions workflow fires, SSHes into my homelab through a Cloudflare Tunnel, and redeploys the app with Docker Compose.

## The Stack

The app is [Dots and Boxes Online](https://dotsandboxesonline.com/), a real-time multiplayer game with a Go backend, React frontend, and WebSockets for live gameplay. It runs inside Docker containers on a Proxmox VM.
    ...
    steps:
      ...
      - name: Setup SSH key
        run: |
          ...
          echo "${{ secrets.HOMELAB_HOST_KEY }}" >> ~/.ssh/known_hosts
      - name: Deploy
        env:
        ...
        run: |
          ssh -i ~/.ssh/deploy_key \
            ...
            "cd /opt/DnBoxes && git pull origin main && docker compose up -d --build"
```
...
The main reason I went with Cloudflare Tunnel over something like portforwarding is because I didn't want to open port 22 on my home network. Even with key-only SSH and fail2ban, an exposed SSH port on a residential IP is a constant target for automated scanners. Cloudflare Tunnel avoids that entirely. The homelab only makes outbound connections, so there's nothing to scan or brute-force from the outside.
